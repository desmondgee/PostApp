  ...
  describe 'GET #list' do
    ...
    context 'less than 20 posts in database' do
      it 'should return a posts array will all current posts' do
        ...
        expect(json['posts'][0]['title']).to eq(post1.title)
        expect(json['posts'][0]['content']).to eq(post1.content)
        expect(json['posts'][0]['author_name']).to eq(user1.name)
        expect(json['posts'][0]['author_city']).to eq(user1.city)
        expect(json['posts'][0]['image_urls']).to be_a(Array)
        expect(json['posts'][0]['image_urls'].length).to eq(2)
        expect(json['posts'][0]['image_urls'][0]).to eq(image1.src)
        expect(json['posts'][0]['image_urls'][1]).to eq(image2.src)
        
        expect(json['posts'][1]['title']).to eq(post2.title)
        expect(json['posts'][1]['content']).to eq(post2.content)
        expect(json['posts'][1]['author_name']).to eq(user2.name)
        expect(json['posts'][1]['author_city']).to eq(user2.city)
        expect(json['posts'][1]['image_urls']).to be_a(Array)
        expect(json['posts'][1]['image_urls'].length).to eq(1)
        expect(json['posts'][1]['image_urls'][0]).to eq(image3.src)
        
        expect(json['posts'][2]['title']).to eq(post3.title)
        expect(json['posts'][2]['content']).to eq(post3.content)
        expect(json['posts'][2]['author_name']).to eq(user1.name)
        expect(json['posts'][2]['author_city']).to eq(user1.city)
        expect(json['posts'][2]['image_urls']).to be_a(Array)
        expect(json['posts'][2]['image_urls'].length).to eq(1)
        expect(json['posts'][2]['image_urls'][0]).to eq(image4.src)
      end
    end
    context 'more than 20 posts in database' do
      it 'should get last 20 posts' do
        ...
        expect(json).to be_a(Hash)
        expect(json['posts']).to be_a(Array)
        expect(json['posts'].length).to eq(20)
        
        expect(json['posts'][0]['title']).to eq(post1.title)
        expect(json['posts'][0]['content']).to eq(post1.content)
        expect(json['posts'][0]['author_name']).to eq(post1.user.name)
        expect(json['posts'][0]['author_city']).to eq(post1.user.city)
        expect(json['posts'][19]['title']).to eq(post20.title)
        expect(json['posts'][19]['content']).to eq(post20.content)
        expect(json['posts'][19]['author_name']).to eq(post20.user.name)
        expect(json['posts'][19]['author_city']).to eq(post20.user.city)
      end
    end
  end
